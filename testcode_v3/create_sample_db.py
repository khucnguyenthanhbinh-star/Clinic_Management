...
def create_sample_data():
    print("--- KHOI TAO DU LIEU TEST CHUAN (FULL TINH NANG) ---")
    db = ClinicDatabase()
    conn = db.conn
    cursor = db.cursor
    ...
    users = [
        ...
        # --- BÁC SĨ (Đa dạng khoa/cơ sở) ---
        ("bs_hung", pw_hash, "doctor", "ThS.BS Trần Văn Hùng", 
         doc_info("Tim mạch", 15, 4.9, 500000, "CS1")), # CS1
         
        ("bs_lien", pw_hash, "doctor", "BS.CKII Nguyễn Thị Liên", 
         doc_info("Nhi khoa", 20, 5.0, 400000, "CS1", "doc_female.png")), # CS1
         
        ("bs_minh", pw_hash, "doctor", "BS. Lê Nhật Minh", 
         doc_info("Da liễu", 8, 4.7, 300000, "CS2")), # CS2
         
        ("bs_thao", pw_hash, "doctor", "ThS.BS Phạm Thanh Thảo", 
         doc_info("Nội tổng quát", 12, 4.8, 250000, "CS2", "doc_female.png")), # CS2

        # --- BỆNH NHÂN ---
        # BN1: Thông tin đầy đủ (Dùng để test chính)
        ...
        # BN2: Thiếu thông tin (Để test chức năng bắt buộc cập nhật)
    ...
    # 4. TẠO LỊCH HẸN & LỊCH SỬ KHÁM (QUAN TRỌNG)
    today = datetime.now()
    ...
    appointments = [
        # --- QUÁ KHỨ (Đã xong - Test xem lịch sử) ---
        ...
        # --- HÔM NAY (Test quy trình khám) ---
        # 3. Đã Check-in (Để test Bác sĩ gọi vào khám) -> Màu vàng cam
        ("bn_an", d_today, "08:00", 
         "[BK03] [CS1] [BS.CKII Nguyễn Thị Liên] [BN: Con trai] Bé ho nhiều về đêm, sốt nhẹ.", 
         "Checked-in"),

        # 4. Mới đặt (Chưa đến) -> Màu xanh
        ("bn_binh", d_today, "10:30", 
         "[BK04] [CS2] [BS. Lê Nhật Minh] Tư vấn da liễu.", 
         "Da dat"),

        # --- TƯƠNG LAI (Test nhắc lịch/Hủy lịch) ---
        # 5. Ngày mai
        ("bn_an", d_tmr, "09:00", 
    ...
    invoices = [
        ...
        # Hóa đơn hôm nay (Chưa trả - Test thanh toán)
    ...
    print("2. Bac si (Tim, CS1): bs_hung / 123456")
    print("3. Benh nhan (Full HS): bn_an / 123456")
    print("4. Benh nhan (Thieu HS): bn_binh / 123456")
