...
class ManageMedicinesView(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # === TREEVIEW ===
        self.tree = ttk.Treeview(self, columns=("medicine", "quantity", "unit"), show="headings", height=12)
        ...
        ttk.Button(btn_frame, text="Làm mới", command=self.refresh_tree).pack(side="left", padx=5)
    ...
    def filter_medicines(self):
        ...
        for name, info in self.all_medicines.items():
            if search_term in name.lower():
                self.tree.insert("", "end", values=(name, info["quantity"], info["unit"]))
