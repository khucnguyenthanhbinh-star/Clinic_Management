...
class ClinicDrugManagementApp:
    ...
    def create_export_tab(self):
        ...
        self.drug_combo = ttk.Combobox(export_form, values=[d["name"] for d in self.drugs], width=40, state="readonly")
    ...
    def export_drug(self):
        try:
            ...
            self.load_drugs()
            self.load_log()
            self.drug_combo["values"] = [d["name"] for d in self.drugs]  # Cập nhật combobox
        except Exception as e:
            messagebox.showerror("Lỗi", str(e))
