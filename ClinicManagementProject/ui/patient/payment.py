...
class PaymentView(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # Tiêu đề
        tk.Label(self, text="THANH TOÁN VIỆN PHÍ", font=("Arial", 16, "bold")).pack(pady=20)
        ...
        tk.Label(left_frame, text="* Giữ phím Ctrl để chọn nhiều hóa đơn cần thanh toán", font=("Arial", 9, "italic"), foreground="gray").pack(pady=5, anchor="w")

        # --- NỘI DUNG CỘT PHẢI ---
        tk.Label(right_frame, text="Tổng thanh toán:", font=("Arial", 10)).pack(anchor="w")
        self.lbl_total = tk.Label(right_frame, text="0 VNĐ", font=("Arial", 18, "bold"), foreground="#d9534f")
        self.lbl_total.pack(anchor="w", pady=(0, 20))
        
        tk.Label(right_frame, text="Phương thức thanh toán:", font=("Arial", 10, "bold")).pack(anchor="w", pady=5)
...
class PaymentProcessingPopup(tk.Toplevel):
    def __init__(self, parent, amount, method, callback):
        ...
        tk.Label(self, text=f"Đang kết nối cổng thanh toán...", font=("Arial", 10)).pack(pady=10)
        tk.Label(self, text=f"Số tiền: {amount:,.0f} VNĐ", font=("Arial", 11, "bold")).pack(pady=5)
        tk.Label(self, text=f"Qua: {method}", font=("Arial", 9)).pack()
