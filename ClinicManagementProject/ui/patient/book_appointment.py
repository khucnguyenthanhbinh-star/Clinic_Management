...
class BookAppointmentView(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        self.booking_data = {
            ...
            "date": "", "time": "",
            "is_relative": False, "patient_name": "", "reason": ""
        }

        self.header_lbl = tk.Label(self, text="ĐẶT LỊCH KHÁM", font=("Arial", 16, "bold"), foreground="#007bff")
    ...
    def show_step_1_clinic(self):
        ...
        self.header_lbl.config(text="CHỌN CƠ SỞ")
        frame = ttk.Frame(self.container)
        frame.pack(fill="both", expand=True)
        tk.Label(frame, text="Vui lòng chọn cơ sở khám:", font=("Arial", 12)).pack(anchor="w", pady=20)
    ...
    def show_step_2_service(self):
        self.clear_container()
        self.header_lbl.config(text="CHỌN DỊCH VỤ")
        ...
        tk.Label(frame, text="Chuyên khoa:", font=("Arial", 11, "bold")).pack(anchor="w")
        ...
        tk.Label(frame, text="Dịch vụ:", font=("Arial", 11, "bold")).pack(anchor="w", pady=(15,5))
        self.services = [("Khám thường", 0), ("Khám VIP", 200000), ("Tư vấn từ xa", -50000)]
    ...
    def show_step_3_doctor(self):
        self.clear_container()
        self.header_lbl.config(text="CHỌN BÁC SĨ")
    ...
    def show_step_4_time(self):
        self.clear_container()
        self.header_lbl.config(text="CHỌN LỊCH KHÁM")
    ...
    def show_step_5_info(self):
        self.clear_container()
        self.header_lbl.config(text="THÔNG TIN BỆNH NHÂN")
        ttk.Button(self.container, text="<< Quay lại", command=self.show_step_4_time).pack(anchor="w")

        f_type = ttk.LabelFrame(self.container, text="Đối tượng khám", padding=10)
        f_type.pack(fill="x", pady=10)
        
        self.rel_var = tk.BooleanVar(value=False)
        ttk.Radiobutton(f_type, text="Đặt cho bản thân (Lấy từ hồ sơ)", variable=self.rel_var, value=False, command=self.toggle_form).pack(anchor="w")
        ttk.Radiobutton(f_type, text="Đặt hộ người thân (Nhập mới)", variable=self.rel_var, value=True, command=self.toggle_form).pack(anchor="w")

        self.f_form = ttk.Frame(f_type)
        self.f_form.pack(fill="x", pady=10)
        
        self.e_name = self.mk_inp("Họ tên (*):", 0)
        self.e_phone = self.mk_inp("SĐT (*):", 1)
        self.e_dob = self.mk_inp("Ngày sinh (*):", 2)
        self.e_cccd = self.mk_inp("CCCD/CMND (*):", 3)
        
        ttk.Label(self.f_form, text="Giới tính (*):").grid(row=4, column=0, sticky="e", pady=5)
        self.cb_gender = ttk.Combobox(self.f_form, values=["Nam", "Nữ"], state="readonly", width=28); self.cb_gender.current(0)
        self.cb_gender.grid(row=4, column=1, pady=5)

        ttk.Label(self.container, text="Triệu chứng (Bắt buộc):").pack(anchor="w", pady=(10,5))
        self.e_reason = ttk.Entry(self.container, width=50); self.e_reason.pack(fill="x")

        ttk.Button(self.container, text="KIỂM TRA & TIẾP TỤC", command=self.save_step_5).pack(pady=20, ipadx=10)
        self.toggle_form()

    def mk_inp(self, lbl, r):
        ttk.Label(self.f_form, text=lbl).grid(row=r, column=0, sticky="e", pady=5, padx=5)
        e = ttk.Entry(self.f_form, width=30); e.grid(row=r, column=1, pady=5)
        return e

    def toggle_form(self):
        st = 'normal' if self.rel_var.get() else 'disabled'
        for w in self.f_form.winfo_children(): w.configure(state=st)

    def save_step_5(self):
        reason = self.e_reason.get().strip()
        if not reason: return messagebox.showerror("Thiếu thông tin", "Vui lòng nhập triệu chứng/lý do khám!")
        self.booking_data["reason"] = reason
        self.booking_data["is_relative"] = self.rel_var.get()

        if self.booking_data["is_relative"]:
            ...
            if not (name and phone and dob and cccd):
                return messagebox.showerror("Thiếu thông tin", "Vui lòng điền ĐẦY ĐỦ: Họ tên, SĐT, Ngày sinh, CCCD của người thân!")
            
            self.booking_data["patient_name"] = f"{name} (Người thân)"
            ...
            if not info.get('cccd'): missing.append("Số CCCD")
            if not info.get('dob'): missing.append("Ngày sinh")
            
            if missing:
                msg = f"Hồ sơ của bạn đang thiếu: {', '.join(missing)}.\n\nTheo quy định Bệnh án điện tử, bạn BẮT BUỘC phải cập nhật đủ thông tin mới được đặt lịch."
                if messagebox.askyesno("Hồ sơ chưa đạt chuẩn", msg + "\n\nBạn có muốn đến trang Cập nhật hồ sơ ngay không?"):
                    from ui.patient.update_info import UpdateInfoView
                    for w in self.master.winfo_children(): w.destroy()
                    UpdateInfoView(self.master, self.controller).pack(fill="both", expand=True)
                return 
            
            self.booking_data["patient_name"] = user['name']

        self.show_step_6_confirm()

    def show_step_6_confirm(self):
        self.clear_container()
        self.header_lbl.config(text="XÁC NHẬN")
        ttk.Button(self.container, text="<< Quay lại", command=self.show_step_5_info).pack(anchor="w")
        
        f_info = ttk.LabelFrame(self.container, text="Phiếu xác nhận", padding=15)
        f_info.pack(fill="both", expand=True, pady=10)
        
        total = self.booking_data["doctor_price"] + self.booking_data["service_price"]
        lines = [
            f"BỆNH NHÂN: {self.booking_data['patient_name']}",
            f"BÁC SĨ: {self.booking_data['doctor_name']}",
            f"THỜI GIAN: {self.booking_data['time']} - {self.booking_data['date']}",
            "---------------------------------------",
            f"Dịch vụ: {self.booking_data['service']}",
            f"TỔNG THANH TOÁN: {total:,.0f} VNĐ"
        ]
        for l in lines: 
            f = ("Arial", 12, "bold") if "TỔNG" in l else ("Courier", 11)
            c = "red" if "TỔNG" in l else "black"
            tk.Label(f_info, text=l, font=f, foreground=c).pack(anchor="w", pady=2)
            
        ttk.Button(self.container, text="XÁC NHẬN & ĐẶT LỊCH", command=lambda: self.finish(total)).pack(pady=20, fill="x", ipady=10)

    def finish(self, total):
        code = f"BK{random.randint(1000,9999)}"
        reason = f"[{code}] [{self.booking_data['clinic_id']}] [{self.booking_data['doctor_name']}] {self.booking_data['reason']}"
        ...
        self.controller.db.add_invoice(self.controller.auth.current_user, f"Phí khám {code}", total, "Unpaid")
